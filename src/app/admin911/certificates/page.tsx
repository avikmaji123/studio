...
export default function AdminCertificatesPage() {
    ...
    const handleDeleteCertificate = async () => {
        if (!certificateToDelete || !firestore || !adminUser) return;

        try {
            // Delete public record
            const publicRef = doc(firestore, 'certificates', certificateToDelete.certificateCode);
            await deleteDoc(publicRef);

            // Delete user's private record
            const userCertRef = doc(firestore, 'users', certificateToDelete.userId, 'certificates', certificateToDelete.courseId);
            await deleteDoc(userCertRef);
            
            await createLogEntry({
                source: 'admin',
                severity: 'warning', // Deletion is a significant action
                message: `Certificate deleted for ${certificateToDelete.studentName}`,
                metadata: { userId: adminUser.uid, certificateCode: certificateToDelete.certificateCode }
            });
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            for <span className="font-bold">{certificateToDelete?.studentName}</span>
                            with code <span className="font-mono">{certificateToDelete?.certificateCode}</span>.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => setCertificateToDelete(null)}>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={handleDeleteCertificate} className={buttonVariants({ variant: "destructive" })}>
