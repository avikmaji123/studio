...
export default function EditCouponPage() {
  ...
  const couponRef = useMemoFirebase(() => doc(firestore, 'coupons', couponId as string), [firestore, couponId]);
  const { data: coupon, isLoading: couponLoading } = useDoc<Coupon>(couponRef);
  ...
  useEffect(() => {
    if (coupon) {
      form.reset({
        ...
        isGlobal: coupon.applicableCourseIds.length === 0,
      });
    }
  }, [coupon, form]);
  ...
  const onSubmit = async (data: CouponFormValues) => {
    if (!firestore || !adminUser || !couponId) return;
    setIsSaving(true);
    ...
    try {
      ...
      const couponDocRef = doc(firestore, 'coupons', couponId as string);
      await updateDoc(couponDocRef, couponPayload);

      await createLogEntry({
        source: 'admin',
        severity: 'info',
        message: `Coupon updated: ${data.code.toUpperCase()}`,
        metadata: { userId: adminUser.uid, couponId: couponId as string },
      });

      toast({ title: 'Coupon Updated', description: `Coupon "${data.code.toUpperCase()}" has been successfully updated.` });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <Card className="max-w-3xl mx-auto">
            <CardHeader>
              ...
              <CardDescription>Update the details for this discount coupon.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                 <FormField control={form.control} name="type" render={({ field }) => (<FormItem><FormLabel>Discount Type</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent><SelectItem value="percentage">Percentage</SelectItem><SelectItem value="fixed">Fixed Amount</SelectItem></SelectContent></Select><FormMessage /></FormItem>)} />
              ...
              <div className="grid md:grid-cols-2 gap-4">
               ...
               <FormField control={form.control} name="status" render={({ field }) => (<FormItem><FormLabel>Status</FormLabel><FormControl><RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-4 pt-2"><FormItem className="flex items-center space-x-3 space-y-0"><FormControl><RadioGroupItem value="active" id="active" /></FormControl><Label htmlFor="active" className="font-normal">Active</Label></FormItem><FormItem className="flex items-center space-x-3 space-y-0"><FormControl><RadioGroupItem value="inactive" id="inactive" /></FormControl><Label htmlFor="inactive" className="font-normal">Inactive</Label></FormItem></RadioGroup></FormControl></FormItem>)} />
                <div>
